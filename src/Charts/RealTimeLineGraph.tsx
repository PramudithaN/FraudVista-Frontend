...
const RealTimeLineGraph: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get<Transaction[]>("http://localhost:8080/allTransaction");
  ...
  useEffect(() => {
    ...
    if (chartData) {
      ...
      if (selectedMonth && selectedYear) {
        filteredData = {
          labels: chartData.labels.filter((date: string) => dayjs(date).format('MM-YYYY') === `${selectedMonth}-${selectedYear}`),
          datasets: chartData.datasets.map((dataset: any) => ({
            ...dataset,
            data: dataset.data.filter((_: any, index: number) => dayjs(chartData.labels[index]).format('MM-YYYY') === `${selectedMonth}-${selectedYear}`)
          })),
        };
      }
  ...
  return (
    <div>
      ...
      <select
          ...
          value={selectedYear || ''}
          style={{ alignSelf: 'flex-start' ,padding:'5px',backgroundColor:'#f2f2f2',borderRadius:'5px',marginRight:'10px'}}
        >
        ...
        <select
          ...
          value={selectedMonth || ''}
          style={{ alignSelf: 'flex-start' ,padding:'5px',backgroundColor:'#f2f2f2',borderRadius:'5px'}}
        >
      ...
      <canvas id="myChart" style={{backgroundColor:'white', width: '100%', maxWidth: '1300px', height: '100%', maxHeight: '480px' ,color:'white',borderRadius:'10px',marginBottom:'10px'}} />
    </div>
  );
};
